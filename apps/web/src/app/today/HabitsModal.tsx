...
export function HabitsModal({ open, onOpenChange }: Props) {
  ...
  const { createMutation, updateMutation, toggleActiveMutation, deleteMutation, reorderMutation } =
    useHabitMutations();
  ...
  function handleSubmit() {
    ...
    if (editingId) {
      updateMutation.mutate({ id: editingId, ...form, name: form.name.trim() }, { onSuccess: resetForm });
    } else {
      createMutation.mutate({ ...form, name: form.name.trim() }, { onSuccess: resetForm });
    }
  }
